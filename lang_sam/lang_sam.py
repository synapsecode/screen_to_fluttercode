...
class LangSAM():
    ...
    def predict(self, image_pil, text_prompt, box_threshold=0.29, text_threshold=0.25):
        ...
        for i, box in enumerate(boxes):
            if phrases[i]=='icon' or phrases[i]=='icon button':
                # box = box.tolist()
                for j, c_box in enumerate(boxes):
                    if i==j:
                        continue
                    if phrases[j]=='text':
                        # c_box = c_box.tolist()
                        if (c_box[0]<=box[0] and c_box[2]>=box[2]) and (c_box[1]-box[3])<=160.0 and (c_box[1]-box[3])>0:
                            new_box = [c_box[0],box[1],c_box[2],c_box[3]]
                            new_boxes.append(new_box)
                            new_phrases.append('icon')
                            # del_icon.append(i)
                            # del_text.append(j)
                        if (c_box[0]>box[0] and c_box[2]<box[2]) and (c_box[1]-box[3])<=160.0 and (c_box[1]-box[3])>0:
                            new_box = [box[0],box[1],box[2],c_box[3]]
                            new_boxes.append(new_box)
                            new_phrases.append('icon')
                            # del_icon.append(i)
                            # del_text.append(j)
        for i, box in enumerate(boxes):
            # if i in del_icon or i in del_text:
            #     continue
        ...
        if len(new_boxes) > 0:
            masks = self.predict_sam(image_pil, new_boxes)
            masks = masks.squeeze(1)
        return masks, new_boxes, new_phrases, logits
